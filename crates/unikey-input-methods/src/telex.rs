...
mod tests {
    ...
    #[test]
    fn test_telex_basic_characters() {
        let mut telex = TelexMethod::new();
        
        // Test 'a' -> wait
        let key_a = KeyEvent::new(KeyEventType::Normal, CharType::Vn, VnLexiName::A, 97);
        let result = telex.process_key(key_a).unwrap();
        assert!(result.is_empty());
        assert_eq!(telex.get_buffer(), "a");
        
        // Test 'aa' -> 'â'
        let key_a2 = KeyEvent::new(KeyEventType::Normal, CharType::Vn, VnLexiName::A, 97);
        let result = telex.process_key(key_a2).unwrap();
        assert_eq!(String::from_utf8_lossy(&result), "â");
        assert!(telex.get_buffer().is_empty());
    }
    
    #[test]
    fn test_telex_diacritics() {
        let mut telex = TelexMethod::new();
        
        // Test 'aw' -> 'ă'
        let key_a = KeyEvent::new(KeyEventType::Normal, CharType::Vn, VnLexiName::A, 97);
        let _ = telex.process_key(key_a);
        let key_w = KeyEvent::new(KeyEventType::Normal, CharType::Vn, VnLexiName::A, 119);
        let result = telex.process_key(key_w).unwrap();
    ...
    fn test_telex_tone_marks() {
        ...
        let result = telex.process_key(key_1).unwrap();
        assert_eq!(String::from_utf8_lossy(&result), "á");
    }
    
    #[test]
    fn test_telex_word_break() {
        let mut telex = TelexMethod::new();
        ...
        let result = telex.process_key(key_space).unwrap();
