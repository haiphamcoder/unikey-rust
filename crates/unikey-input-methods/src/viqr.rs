...
/// VIQR input method state
#[derive(Debug, Clone)]
pub struct ViqrState {
...
mod tests {
    ...
    #[test]
    fn test_viqr_basic_characters() {
        let mut viqr = ViqrMethod::new();
        
        // Test 'a' -> wait
        let key_a = KeyEvent::new(KeyEventType::Normal, CharType::Vn, VnLexiName::A, 97);
        let result = viqr.process_key(key_a).unwrap();
        assert!(result.is_empty());
        assert_eq!(viqr.get_buffer(), "a");
        
        // Test 'a^' -> 'â'
        let key_caret = KeyEvent::new(KeyEventType::Normal, CharType::Vn, VnLexiName::A, 94);
        let result = viqr.process_key(key_caret).unwrap();
        assert_eq!(String::from_utf8_lossy(&result), "â");
        assert!(viqr.get_buffer().is_empty());
    }
    
    #[test]
    fn test_viqr_diacritics() {
        let mut viqr = ViqrMethod::new();
        
        // Test 'a(' -> 'ă'
        let key_a = KeyEvent::new(KeyEventType::Normal, CharType::Vn, VnLexiName::A, 97);
        let _ = viqr.process_key(key_a);
        let key_paren = KeyEvent::new(KeyEventType::Normal, CharType::Vn, VnLexiName::A, 40);
        let result = viqr.process_key(key_paren).unwrap();
        assert_eq!(String::from_utf8_lossy(&result), "ă");
    }
    
    #[test]
    fn test_viqr_tone_marks() {
        let mut viqr = ViqrMethod::new();
        
        // Test 'a\'' -> 'á'
        let key_a = KeyEvent::new(KeyEventType::Normal, CharType::Vn, VnLexiName::A, 97);
        let _ = viqr.process_key(key_a);
        let key_apostrophe = KeyEvent::new(KeyEventType::Normal, CharType::Vn, VnLexiName::A, 39);
        let result = viqr.process_key(key_apostrophe).unwrap();
        assert_eq!(String::from_utf8_lossy(&result), "á");
    }
    
    #[test]
    fn test_viqr_word_break() {
        let mut viqr = ViqrMethod::new();
        ...
        let result = viqr.process_key(key_space).unwrap();
